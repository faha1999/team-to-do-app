...
type Member = { id: string; name: string; role: TeamRole };
...
export default function TeamSettingsPage({
  ...
  const [description, setDescription] = useState('Core, platform, infra.');
  const [visibility, setVisibility] = useState<Visibility>('PUBLIC');
  const [members, setMembers] = useState<Member[]>([
    { id: 'u1', name: 'You', role: 'OWNER' },
    { id: 'u2', name: 'Fahad', role: 'ADMIN' },
    { id: 'u3', name: 'Amira', role: 'MEMBER' },
    { id: 'u4', name: 'Sara', role: 'GUEST' },
  ]);

  const roles: TeamRole[] = ['OWNER', 'ADMIN', 'MEMBER', 'GUEST'];

  function save() {
    alert('Saved team settings (wire to Server Action/Prisma).');
  }
  function invite() {
    const name = prompt('Invite who?');
    if (!name) return;
    setMembers((xs) => [
      ...xs,
      { id: crypto.randomUUID(), name, role: 'GUEST' },
    ]);
  }
      ...
      <section className="mx-auto max-w-4xl px-6 py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-zinc-500">
              Centralized admin controls for permissions & data access.
            </p>
          </div>
          <Link
            href={`/teams/${teamId}`}
        ...
        <div className="space-y-6">
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-[0_10px_30px_-15px_rgba(0,0,0,0.25)]">
            <div className="flex items-center justify-between border-b border-zinc-200 bg-zinc-50/60 px-4 py-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-zinc-500">
                  Owners/Admins can manage access & roles.
                </p>
              </div>
              <button
                onClick={invite}
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-600">
            <p className="font-medium text-zinc-900">Data ownership</p>
            <p className="mt-1">
              Team projects belong to the team. Personal projects belong to
              individuals — even if they leave the team.
            </p>
          </div>
        </div>
      </section>
